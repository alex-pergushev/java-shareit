...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findAllByBookerIdAndEndBeforeOrderByStartDesc(
            Long id, LocalDateTime end); //все завершенные бронирования

    List<Booking> findAllByBookerIdAndStartAfterOrderByStartDesc(
            Long id, LocalDateTime start); //все будущие бронирования
    ...
    @Query(value = "select * from bookings b join items i on b.item_id=i.id " +
            "where owner_id = :id and start_date < :start and end_date > :end order by start_date desc",
            nativeQuery = true)
    ...
    List<Booking> findAllByItemIdInAndStatusOrderByStartDesc(
            Long id, String status); //все бронирования в статусе ожидания подтверждения
